...
module.exports = async (scope, cacheKeyName, forceNew = false) => {
  const cacheKey = `graphToken${cacheKeyName}`
  ...
  const config = {
    auth: {
      clientId: graphClient.GRAPH_CLIENT_ID,
      authority: `https://login.microsoftonline.com/${graphClient.GRAPH_TENANT_ID}/`,
      clientSecret: graphClient.GRAPH_CLIENT_SECRET
    }
  }
