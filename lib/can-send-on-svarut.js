...
module.exports = (documentData, exceptions) => {
  ...
  if (documentData.flowStatus.syncEnterprise && documentData.variant === 'bekreftelse-bedrift') {
    if (documentData.flowStatus.syncEnterprise.result.enterprise.PostAddress.ZipCode.length !== 4) throw new Error('Enterprise ZipCode is not 4 digits!! Please edit in json-file, and run again')
